...
pub fn derive_query_for_enum(ast: &DeriveInput, enum_data: &DataEnum) -> TokenStream {
            ...
            let vname = match &variant.fields {
                Fields::Unnamed(f) => {
                    match &f.unnamed[0].ty {
                        ...
                        _ => panic!("#[derive(Query) operates on wrapped struct, such as QueryName(QueryStruct), but none was found")
                    }
                },
                _ => panic!("#[derive(Query) operates on wrapped struct, such as QueryName(QueryStruct), but none was found")
            };
    ...
    (quote! {
        ...
        impl #impl_generics ::base::domain::entity::query::Query for #tname #ty_generics #where_clause {
